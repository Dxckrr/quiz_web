export default class BooksTemplate {
    ...
    render = async () => {
        const booksTemplate = Promise.all(this.books.map(async (book) => {
            return `
            <article class="card">
                <div class="card-aside">
                    <img src="${await this.getImage(book._pt)}" alt='this is an ${book._pt}'>
                </div>
                <div class="content">
                    <h1>${book._title}</h1>
                    <p>${book._author}</p>
    ...
    getImage = async (image) => {
        const path = `./img/`;
        const imagePath = `${path}${image.replace(/@/g, '')}.png`;
